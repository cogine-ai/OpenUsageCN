    ...
    return parts.map((part, i) => {
      ...
      if (part.type === "pr") {
        return (
          <button
            key={i}
            onClick={() => openUrl(`https://github.com/cogine-ai/OpenUsageCN/pull/${part.content.slice(1)}`).catch(console.error)}
            className={linkClass}
          >
            {part.content}
          </button>
      ...
      if (part.type === "commit") {
        return (
          <button
            key={i}
            onClick={() => openUrl(`https://github.com/cogine-ai/OpenUsageCN/commit/${part.content}`).catch(console.error)}

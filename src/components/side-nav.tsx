  ...
  const handlePluginContextMenu = useCallback(
    (e: React.MouseEvent, pluginId: string) => {
      ...
      ;(async () => {
        ...
        const menuItems = [reloadItem, removeItem]
        const closeTargets: Array<MenuItem | PredefinedMenuItem> = [reloadItem, removeItem]
        ...
        if (import.meta.env.DEV) {
          ...
          closeTargets.push(bottomSeparator, inspectItem)
        }
        ...
        try {
          ...
          await Promise.allSettled([menu.close(), ...closeTargets.map((item) => item.close())])
        }
      })().catch(console.error)
    },

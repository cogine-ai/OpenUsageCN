  ...
  const handlePluginContextMenu = useCallback(
    (e: React.MouseEvent, pluginId: string) => {
      ...
      ;(async () => {
        ...
        const closePromises: Promise<unknown>[] = [reloadItem.close(), removeItem.close()]
        ...
        if (import.meta.env.DEV) {
          ...
          closePromises.push(bottomSeparator.close(), inspectItem.close())
        }
        ...
        try {
          ...
          await Promise.allSettled([menu.close(), ...closePromises])
        }
      })().catch(console.error)
    },

...
function CogineMark({ className }: { className?: string }) {
  return (
    <span
      aria-hidden="true"
      className={cn("inline-block shrink-0 bg-current", className)}
      style={{
        WebkitMaskImage: "url(/cogine-mark-48.png)",
        WebkitMaskSize: "contain",
        WebkitMaskRepeat: "no-repeat",
        WebkitMaskPosition: "center",
        maskImage: "url(/cogine-mark-48.png)",
        maskSize: "contain",
  ...
  const handlePluginContextMenu = useCallback(
    (e: React.MouseEvent, pluginId: string) => {
      ...
      ;(async () => {
        ...
        const menuItems: Array<Awaited<ReturnType<typeof MenuItem.new>> | Awaited<ReturnType<typeof PredefinedMenuItem.new>>> = [
          reloadItem,
          removeItem,
        ]
        const closables: Array<Awaited<ReturnType<typeof MenuItem.new>> | Awaited<ReturnType<typeof PredefinedMenuItem.new>>> = [
          reloadItem,
          removeItem,
        ]
        if (import.meta.env.DEV) {
          ...
          closables.push(bottomSeparator, inspectItem)
        }
  ...
  return (
    <nav className="flex flex-col w-12 border-r bg-muted/50 dark:bg-card py-3">
      ...
      <NavButton
        ...
        aria-label="总览"
      >
        <CogineMark className="size-6 dark:text-page-accent" />
      </NavButton>

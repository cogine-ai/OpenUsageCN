(function () {
  const KEYCHAIN_SERVICE = "OpenUsageCN-copilot";
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed && parsed.token) {
          ctx.host.log.info("token loaded from OpenUsageCN keychain");
          return { token: parsed.token, source: "keychain" };
        }
      }
    } catch (e) {
      ctx.host.log.info("OpenUsageCN keychain read failed: " + String(e));
    }
    return null;
  }
  ...
  function probe(ctx) {
    ...
    // Persist gh-cli token to OpenUsageCN keychain for future use
    if (source === "gh-cli") {
      saveToken(ctx, token);
    }

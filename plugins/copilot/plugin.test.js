...
function setKeychainToken(ctx, token) {
  ctx.host.keychain.readGenericPassword.mockImplementation((service) => {
    if (service === "OpenUsageCN-copilot") return JSON.stringify({ token });
    return null;
  });
}
...
describe("copilot plugin", () => {
  ...
  it("loads token from OpenUsageCN keychain", async () => {
    const ctx = makePluginTestContext();
  ...
  it("does not persist token loaded from OpenUsageCN keychain", async () => {
    const ctx = makePluginTestContext();
  ...
  it("falls back when OpenUsageCN keychain payload lacks token field", async () => {
    const ctx = makePluginTestContext();
    ctx.host.keychain.readGenericPassword.mockImplementation((service) => {
      if (service === "OpenUsageCN-copilot") return JSON.stringify({ notToken: "x" });

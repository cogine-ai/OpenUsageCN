(function () {
  ...
  function loadApiKey(ctx) {
    ...
    const zhipu = ctx.host.env.get("ZHIPUAI_API_KEY")
    if (typeof zhipu === "string" && zhipu.trim()) return zhipu.trim()
  ...
  function fetchQuota(ctx, apiKey) {
    ...
    if (ctx.util.isAuthStatus(resp.status)) {
      throw "API key invalid. Check your BigModel CN API key."
    }
  ...
  globalThis.__openusage_plugin = { id: "bigmodel-cn", probe }
})()

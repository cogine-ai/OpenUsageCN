...
#[test]
#[serial]
fn route_probe_only_limits_provider_returns_200_with_redacted_errors() {
    ...
    assert!(resp.contains(r#""schema":"openusage.limits.v1""#));
    assert!(resp.contains(r#""providerId":"codex""#) || resp.contains(r#"\"providerId\":\"codex\""#));
    assert!(resp.contains(r#""errors""#));
    assert!(
        !resp.contains(jwt),
        "limits HTTP response must not leak raw JWT probe errors"
    );
}

#[test]
#[serial]
fn route_cached_limits_provider_returns_200_with_partial_resource_errors() {
    ...
    };
    {
        ...
        state
            .snapshots
            .insert("codex".to_string(), snapshot);
    ...
    assert!(resp.contains(r#"count resource is missing a stable unit"#));

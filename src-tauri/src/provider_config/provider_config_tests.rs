...
fn delete_rejects_unknown_field() {
    ...
    let result = delete_plugin_field_from_path(&path, "bigmodel-cn", &fields, "unknown");
    let _ = std::fs::remove_dir_all(&dir);
    replace_store_for_test(default_file());

    let err = result.expect_err("delete should fail");
    assert!(
        err.contains("Unknown config field 'unknown'"),
        "unexpected error: {err}"
    );
}
...
fn save_rejects_invalid_select_value() {
    ...
    let input = HashMap::from([(
        "region".to_string(),
        Value::String("invalid-region".to_string()),
    )]);
    let result = save_plugin_values_to_path(&path, "zai", &fields, input);
    let _ = std::fs::remove_dir_all(&dir);
    replace_store_for_test(default_file());

    let err = result.expect_err("save should fail");
    assert!(
        err.contains("Invalid value 'invalid-region' for config field 'region'"),
        "unexpected error: {err}"
    );
}

...
- 托盘小面板文案要短，优先使用“刷新用量”“重置时间”“已暂停”这类直接说法。
- 平台入口要准确：macOS 使用“菜单栏”，Windows 使用“系统托盘”；泛指两者时使用“托盘入口”或直接说“OpenUsageCN 图标”。
- Windows MVP 不显示暂不支持的入口，包括 CLI、额度通知、全局快捷键、动态托盘图标和托盘标题设置。

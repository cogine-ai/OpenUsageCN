...
Keychain fallback is available on macOS only. If Codex changes `auth.json` while OpenUsageCN is refreshing a token, OpenUsageCN stops instead of overwriting the newer file.

...
Use this when OpenUsageCN is not working and you need to share debug info.
...
1. In your macOS menu bar, find the OpenUsageCN icon.
2. Right-click it (or hold `Control` and click).
3. Open `Debug Level`.
4. Select `Debug`.

If OpenUsageCN does not open at all, skip this step and continue.
...
1. Attach `openusagecn.log`.
2. If you also see files like `openusagecn.log.1`, attach those too.

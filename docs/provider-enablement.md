...
The saved provider order records which providers this installation has already seen. Once a provider has been checked, the app will not turn it on again automatically. Manually turning a provider off always sticks. If you add credentials later, turn the provider on in Settings. Shell lookup has a five-second limit. If it fails or times out, the app logs the error, saves providers it can confirm from other local sources, and leaves unresolved providers unseen so the next launch can retry. A region selected through an environment variable must be confirmed before Moonshot or Doubao is enabled. If the detection command fails entirely, none of its new provider checks are saved. Changing provider settings can mark unresolved providers as seen before that retry.

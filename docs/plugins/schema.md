...
Saved provider values live in `~/.openusagecn/providers.json`, outside the plugin directory. Bundled plugins are copied into the app data plugin directory on startup and may overwrite plugin files, so user-provided config must not be stored inside `plugins/<id>/`.

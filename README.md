...
- **故障提示。** Codex、Claude 或 Cursor 报告服务异常时，在用量卡片中显示状态页提示。
- **[额度通知](docs/pace-notifications.md)。** 可按需开启即将用尽、接近上限和预计提前用尽三类 macOS 系统通知。
...
- **[全局 CLI](docs/cli.md)。** 安装 `openusage` 命令后，即使菜单栏应用没有运行，脚本和本地智能体也能读取或刷新额度。
